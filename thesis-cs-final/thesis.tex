...
\section{Zdrojový kód a data pro experimentální část}
V příloze práce jsou k dispozici hlavní zdrojové kódy v jazyce Python, grafy a tabulky s výsledky experimentální části a všechny výstupy našeho nástroje použité při testování. Součástí přílohy jsou také grafy, které z důvodu úspory místa nebyly zařazeny do hlavního textu práce. Aktuální verze nástroje a případné budoucí výsledky dalších testování budou dostupné na \url{https://github.com/MartinStruk/symmetries}. Následuje podrobnější popis jednotlivých částí přílohy.

\begin{enumerate}
        \item \texttt{plots}: Všechny vygenerované grafy s výsledky. Význam jednotlivých grafů je popsán v grafech samotných.
        \item \texttt{problems}: Všechny testované výstupy, jejichž výsledky byly srovnávány v experimentální části.
        \item \texttt{tables}: Tabulky se souhrny výsledků z experimentální části.
	\item \texttt{geninput.py}: Zdrojový kód nástroje pro generování vstupních instancí uvažovaných problémů.
	\item \texttt{sym.py}: Zdrojový kód hlavního nástroje práce, který pro zadanou formuli vygeneruje klauzule pro rozbíjení symetrií.
    \end{enumerate} 

Spuštění \texttt{sym.py} nebo \texttt{geninput.py} se provádí z příkazové řádky se specifickými argumenty. Konkrétní pořadí a formát argumentů lze zjistit spuštěním kódu s argumentem \texttt{-h}.
\end{document}
